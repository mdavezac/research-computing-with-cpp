...
\begin{Highlighting}[]
\KeywordTok{template} \NormalTok{<}\KeywordTok{typename} \NormalTok{T>}
\KeywordTok{class} \NormalTok{MyPair \{}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{cmake_minimum_required}\NormalTok{(}\OtherTok{VERSION} \NormalTok{2.8)}

\KeywordTok{find_package}\NormalTok{(MPI }\OtherTok{REQUIRED}\NormalTok{)}

\KeywordTok{include_directories}\NormalTok{(}\DecValTok{$\{MPI_C_INCLUDE_PATH\}}\NormalTok{)}
\KeywordTok{set}\NormalTok{(CMAKE_CXX_FLAGS }\StringTok{"}\DecValTok{$\{CMAKE_CXX_FLAGS\}}\StringTok{ }\DecValTok{$\{MPI_C_COMPILE_FLAGS\}}\StringTok{"}\NormalTok{)}

\KeywordTok{add_executable}\NormalTok{(hello hello.cc)}
\KeywordTok{target_link_libraries}\NormalTok{(hello }\DecValTok{$\{MPI_C_LIBRARIES\}} \DecValTok{$\{MPI_CXX_LIBRARIES\}}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
Running MPI unit-tests requires MPI\_Init and MPI\_Failure before and
...
\subsubsection{Serial Smooth Life}\label{serial-smooth-life}
...
\DataTypeTok{void} \NormalTok{Smooth::QuickUpdate() \{}
  \KeywordTok{for} \NormalTok{(}\DataTypeTok{int} \NormalTok{x=}\DecValTok{0}\NormalTok{;x<sizex;x++) \{}
    \KeywordTok{for} \NormalTok{(}\DataTypeTok{int} \NormalTok{y=}\DecValTok{0}\NormalTok{;y<sizey;y++) \{}
      ...
      \KeywordTok{for} \NormalTok{(}\DataTypeTok{int} \NormalTok{x1=}\DecValTok{0}\NormalTok{;x1<sizex;x1++) \{}
          \DataTypeTok{int} \NormalTok{deltax=TorusDifference(x,x1,sizex);}
          \KeywordTok{if} \NormalTok{(deltax>outer+smoothing/}\DecValTok{2}\NormalTok{) }\KeywordTok{continue}\NormalTok{;}
          ...
          \KeywordTok{for} \NormalTok{(}\DataTypeTok{int} \NormalTok{y1=}\DecValTok{0}\NormalTok{;y1<sizey;y1++) \{}
            ...
            \KeywordTok{if} \NormalTok{(deltay>outer+smoothing/}\DecValTok{2}\NormalTok{) }\KeywordTok{continue}\NormalTok{;}
      ...
      \NormalTok{(*fieldNew)[x][y]=transition(disk_total/normalisation_disk,ring_total/normalisation_ring);}
    \NormalTok{\}}
  \NormalTok{\}}
\end{Highlighting}
\end{Shaded}

\subsubsection{Swapped before/after
fields}\label{swapped-beforeafter-fields}

\begin{Shaded}
\begin{Highlighting}[]

  \NormalTok{std::vector<std::vector<density> > * fieldTemp;}
  \NormalTok{fieldTemp=field;}
  \NormalTok{field=fieldNew;}
  \NormalTok{fieldNew=fieldTemp;}
  \NormalTok{frame++;}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
\DataTypeTok{int} \NormalTok{Smooth::TorusDifference(}\DataTypeTok{int} \NormalTok{x1, }\DataTypeTok{int} \NormalTok{x2, }\DataTypeTok{int} \NormalTok{size) }\DataTypeTok{const} \NormalTok{\{}
    \DataTypeTok{int} \NormalTok{straight=std::abs(x2-x1);}
    \DataTypeTok{int} \NormalTok{wrapleft=std::abs(x2-x1+size);}
    \DataTypeTok{int} \NormalTok{wrapright=std::abs(x2-x1-size);}
    \KeywordTok{if} \NormalTok{((straight<wrapleft) && (straight<wrapright)) \{}
      \KeywordTok{return} \NormalTok{straight;}
    \NormalTok{\} }\KeywordTok{else} \NormalTok{\{}
      \KeywordTok{return} \NormalTok{(wrapleft < wrapright) ? wrapleft : wrapright;}
    \NormalTok{\}}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
    ...
    \NormalTok{range(outer+smoothing/}\DecValTok{2}\NormalTok{),}
    \NormalTok{local_x_size(sizex/mpi_size),}
    \NormalTok{local_x_size_with_halo(local_x_size}\DecValTok{+2}\NormalTok{*range),}
    \NormalTok{total_x_size(sizex),}
    \NormalTok{x_coordinate_offset(rank*local_x_size-range),}
    \NormalTok{local_x_min_calculate(range),}
    \NormalTok{local_x_max_needed_left(}\DecValTok{2}\NormalTok{*range),}
    \NormalTok{local_x_max_calculate(range+local_x_size),}
  ...
  \KeywordTok{for} \NormalTok{(}\DataTypeTok{int} \NormalTok{x=from_x;x<to_x;x++) \{}
    \KeywordTok{for} \NormalTok{(}\DataTypeTok{int} \NormalTok{y=}\DecValTok{0}\NormalTok{;y<sizey;y++) \{}
      ...
      \KeywordTok{for} \NormalTok{(}\DataTypeTok{int} \NormalTok{x1=}\DecValTok{0}\NormalTok{;x1<local_x_size_with_halo;x1++) \{}
          \DataTypeTok{int} \NormalTok{deltax=TorusDifference(x+x_coordinate_offset,x1+x_coordinate_offset,total_x_size);}
          \KeywordTok{if} \NormalTok{(deltax>outer+smoothing/}\DecValTok{2}\NormalTok{) }\KeywordTok{continue}\NormalTok{;}
      ...
      \NormalTok{SetNewField(x,y,transition(disk_total/normalisation_disk,ring_total/normalisation_ring));}
    \NormalTok{\}}
  \NormalTok{\}}
\NormalTok{\}}
\end{Highlighting}
...
\DataTypeTok{void} \NormalTok{Smooth::CommunicateMPI()\{}
  ...
  \NormalTok{MPI_Sendrecv(send_transport_buffer,range*sizey,MPI_DOUBLE,left,rank,}
      \NormalTok{receive_transport_buffer, range*sizey,MPI_DOUBLE,right,right,}
...
\DataTypeTok{void} \NormalTok{Smooth::CommunicateLocal(Smooth &left, Smooth &right)\{}
  \NormalTok{BufferLeftHaloForSend();}
  \NormalTok{std::memcpy(left.receive_transport_buffer,send_transport_buffer,}\KeywordTok{sizeof}\NormalTok{(density)*range*sizey);}
  \NormalTok{left.UnpackRightHaloFromReceive();}
  \NormalTok{BufferRightHaloForSend();}
  \NormalTok{std::memcpy(right.receive_transport_buffer,send_transport_buffer,}\KeywordTok{sizeof}\NormalTok{(density)*range*sizey);}
...
\NormalTok{TEST_CASE (}\StringTok{"CommunicationBufferingFunctionsCorrectly"}\NormalTok{) \{}
  ...
  \NormalTok{smooth.SeedDisk(); }\CommentTok{// Half the Seeded Disk falls in smooth2's domain, so total filling will be half a disk.}
...
\begin{Highlighting}[]
       ...
       \KeywordTok{for} \NormalTok{(}\DataTypeTok{unsigned} \DataTypeTok{int} \NormalTok{x=}\DecValTok{0}\NormalTok{;x<}\DecValTok{50}\NormalTok{;x++)\{}
         \KeywordTok{for} \NormalTok{(}\DataTypeTok{unsigned} \DataTypeTok{int} \NormalTok{y=}\DecValTok{0}\NormalTok{;y<}\DecValTok{100}\NormalTok{;y++)\{}
           \NormalTok{REQUIRE(std::abs(smooth.Field(x}\DecValTok{+15}\NormalTok{+rank*}\DecValTok{50}\NormalTok{,y) - paraSmooth.Field(x}\DecValTok{+15}\NormalTok{,y))<}\FloatTok{0.00001}\NormalTok{);}
         \NormalTok{\}}
       \NormalTok{\}}
    \NormalTok{\}}
\end{Highlighting}
...
\NormalTok{density Smooth::Field(}\DataTypeTok{int} \NormalTok{x,}\DataTypeTok{int} \NormalTok{y) }\DataTypeTok{const} \NormalTok{\{}
  \KeywordTok{return} \NormalTok{(*field)[sizey*x+y];}
\NormalTok{\};}

\DataTypeTok{void} \NormalTok{Smooth::SetNewField(}\DataTypeTok{int} \NormalTok{x, }\DataTypeTok{int} \NormalTok{y, density value)\{}
  \NormalTok{(*fieldNew)[sizey*x + y]=value;}
\NormalTok{\}}

\DataTypeTok{void} \NormalTok{Smooth::SetField(}\DataTypeTok{int} \NormalTok{x, }\DataTypeTok{int} \NormalTok{y, density value)\{}
  \NormalTok{(*field)[sizey*x + y]=value;}
\NormalTok{\}}
...
\DataTypeTok{void} \NormalTok{Smooth::CommunicateMPIUnbuffered()\{}
  \NormalTok{MPI_Sendrecv((*field)+sizey*range,range*sizey,MPI_DOUBLE,left,rank,}
      ...
      \NormalTok{MPI_COMM_WORLD,MPI_STATUS_IGNORE);}
  \NormalTok{MPI_Sendrecv((*field)+sizey*local_x_min_needed_right,range*sizey,MPI_DOUBLE,right,mpi_size+rank,}
      \NormalTok{(*field),range*sizey,MPI_DOUBLE,left,mpi_size+left,}
      \NormalTok{MPI_COMM_WORLD,MPI_STATUS_IGNORE);}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
\DataTypeTok{void} \NormalTok{Smooth::CommunicateMPIDerivedDatatype()\{}
  \NormalTok{MPI_Sendrecv(*field+sizey*local_x_min_calculate,}\DecValTok{1}\NormalTok{,halo_type,left,rank,}
               \NormalTok{*field+sizey*local_x_max_calculate,}\DecValTok{1}\NormalTok{,halo_type,right,right,}
               \NormalTok{MPI_COMM_WORLD,MPI_STATUS_IGNORE);}
  \NormalTok{MPI_Sendrecv(*field+sizey*local_x_min_needed_right,}\DecValTok{1}\NormalTok{,halo_type,right,mpi_size+rank,}
                                              \NormalTok{*field,}\DecValTok{1}\NormalTok{,halo_type,left,mpi_size+left,}
                                              \NormalTok{MPI_COMM_WORLD,MPI_STATUS_IGNORE);}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
\DataTypeTok{void} \NormalTok{Smooth::InitiateLeftComms()\{}
  \NormalTok{MPI_Isend(*field+sizey*local_x_min_calculate,}\DecValTok{1}\NormalTok{,halo_type,left,rank,MPI_COMM_WORLD,&request_left);}
  \NormalTok{MPI_Irecv(*field+sizey*local_x_max_calculate,}\DecValTok{1}\NormalTok{,halo_type,right,right,MPI_COMM_WORLD,&request_left);}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
\DataTypeTok{void} \NormalTok{Smooth::ResolveLeftComms()\{}
  \NormalTok{MPI_Wait(&request_left,MPI_STATUS_IGNORE);}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{density * Smooth::StartOfWritingBlock()\{}
   \KeywordTok{return} \NormalTok{&(*field)[local_x_min_calculate*sizey];}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
\DataTypeTok{void} \NormalTok{TextWriter::Header(}\DataTypeTok{int} \NormalTok{frames)\{}
  \NormalTok{*outfile << smooth.LocalXSize() << }\StringTok{", "} \NormalTok{<< smooth.Sizey() << }\StringTok{", "} \NormalTok{<< rank << }\StringTok{", "} \NormalTok{<< size << std::endl;}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
\DataTypeTok{void} \NormalTok{XDRWriter::Write() \{}
  ...
  \NormalTok{xdr_vector(&xdrfile,start_to_write,local_element_count,}\KeywordTok{sizeof}\NormalTok{(}\DataTypeTok{double}\NormalTok{),}\KeywordTok{reinterpret_cast}\NormalTok{<xdrproc_t>(xdr_double));}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
\DataTypeTok{void} \NormalTok{SingleWriter::Write() \{}
  ...
  \NormalTok{MPI_Gather(smooth.StartOfWritingBlock(),local_element_count,MPI_DOUBLE,}
      \NormalTok{receive_buffer,local_element_count,MPI_DOUBLE,}\DecValTok{0}\NormalTok{,MPI_COMM_WORLD); }

  \KeywordTok{if} \NormalTok{(rank==}\DecValTok{0}\NormalTok{)\{}
     \NormalTok{xdr_vector(&xdrfile,}\KeywordTok{reinterpret_cast}\NormalTok{<}\DataTypeTok{char}\NormalTok{*>(receive_buffer),}
         \NormalTok{total_element_count,}\KeywordTok{sizeof}\NormalTok{(}\DataTypeTok{double}\NormalTok{),}\KeywordTok{reinterpret_cast}\NormalTok{<xdrproc_t>(xdr_double));}
  \NormalTok{\}}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{SingleWriter::SingleWriter(Smooth & smooth, }\DataTypeTok{int} \NormalTok{rank, }\DataTypeTok{int} \NormalTok{size)}
    \NormalTok{:SmoothWriter(smooth,rank,size)}
\NormalTok{\{}
     \KeywordTok{if} \NormalTok{(rank!=}\DecValTok{0}\NormalTok{) \{}
...
\begin{Highlighting}[]
  ...
  \NormalTok{MPI_File_write(outfile,smooth.StartOfWritingBlock(),local_element_count,MPI_DOUBLE,MPI_STATUS_IGNORE);}
\end{Highlighting}
\end{Shaded}
...
\subsection{GPU-accelerated Libraries}\label{gpu-accelerated-libraries}
...
\subsubsection{SAXPY from cuBLAS}\label{saxpy-from-cublas}

\begin{itemize}
\itemsep1pt\parskip0pt\parsep0pt
\item
  The following code snippet replaces the \texttt{saxpy\_fast} function
  with the equivalent \texttt{cublasSaxpy}:
\end{itemize}

\begin{Shaded}
\begin{Highlighting}[]

    \CommentTok{// Allocate vectors on GPU}
    \DataTypeTok{float} \NormalTok{* dx, * dy;}
    \NormalTok{CUDA_ERROR_CHECK(cudaMalloc((}\DataTypeTok{void} \NormalTok{**)&dx, n * incx * }\KeywordTok{sizeof}\NormalTok{(}\DataTypeTok{float}\NormalTok{)));}
    \NormalTok{CUDA_ERROR_CHECK(cudaMalloc((}\DataTypeTok{void} \NormalTok{**)&dy, n * incy * }\KeywordTok{sizeof}\NormalTok{(}\DataTypeTok{float}\NormalTok{)));}

    \CommentTok{// Create CUBLAS handle}
    \NormalTok{cublasHandle_t handle;}
    \NormalTok{CUBLAS_ERROR_CHECK(cublasCreate(&handle));}

    \CommentTok{// Copy vectors into GPU memory}
    \NormalTok{CUBLAS_ERROR_CHECK(cublasSetVector(n, }\KeywordTok{sizeof}\NormalTok{(}\DataTypeTok{float}\NormalTok{), x, incx, dx, incx));}
    \NormalTok{CUBLAS_ERROR_CHECK(cublasSetVector(n, }\KeywordTok{sizeof}\NormalTok{(}\DataTypeTok{float}\NormalTok{), y, incy, dy, incy));}

    \CommentTok{// Perform the GPU SAXPY}
    \NormalTok{CUBLAS_ERROR_CHECK(cublasSaxpy(handle, n, &a, dx, incx, dy, incy));}

    \CommentTok{// Copy results back into CPU memory}
    \NormalTok{CUBLAS_ERROR_CHECK(cublasGetVector(n, }\KeywordTok{sizeof}\NormalTok{(}\DataTypeTok{float}\NormalTok{), dy, incy, y, incy));}
\end{Highlighting}
\end{Shaded}
...
\subsubsection{Why isn't it faster?}\label{why-isnt-it-faster}
...
\begin{Highlighting}[]
    ...
    \CommentTok{// Allocate matrices on GPU}
    \DataTypeTok{float} \NormalTok{* dA, * dB, * dC;}
    \NormalTok{size_t dlda, dldb, dldc;}
    \NormalTok{CUDA_ERROR_CHECK(cudaMallocPitch((}\DataTypeTok{void} \NormalTok{**)&dA, &dlda, m * }\KeywordTok{sizeof}\NormalTok{(}\DataTypeTok{float}\NormalTok{), k));}
    \NormalTok{CUDA_ERROR_CHECK(cudaMallocPitch((}\DataTypeTok{void} \NormalTok{**)&dB, &dldb, k * }\KeywordTok{sizeof}\NormalTok{(}\DataTypeTok{float}\NormalTok{), n));}
    \NormalTok{CUDA_ERROR_CHECK(cudaMallocPitch((}\DataTypeTok{void} \NormalTok{**)&dC, &dldc, m * }\KeywordTok{sizeof}\NormalTok{(}\DataTypeTok{float}\NormalTok{), n));}

    \CommentTok{// cudaMallocPitch returns leading dimensions in bytes while CUBLAS expects}
    \CommentTok{// them as number of elements}
    \NormalTok{dlda /= }\KeywordTok{sizeof}\NormalTok{(}\DataTypeTok{float}\NormalTok{);}
    \NormalTok{dldb /= }\KeywordTok{sizeof}\NormalTok{(}\DataTypeTok{float}\NormalTok{);}
    \NormalTok{dldc /= }\KeywordTok{sizeof}\NormalTok{(}\DataTypeTok{float}\NormalTok{);}

    \CommentTok{// Create CUBLAS handle}
    \NormalTok{cublasHandle_t handle;}
    \NormalTok{CUBLAS_ERROR_CHECK(cublasCreate(&handle));}

    \CommentTok{// Copy matrices into GPU memory}
    \NormalTok{CUBLAS_ERROR_CHECK(cublasSetMatrix(m, k, }\KeywordTok{sizeof}\NormalTok{(}\DataTypeTok{float}\NormalTok{), A, lda, dA, dlda));}
    \NormalTok{CUBLAS_ERROR_CHECK(cublasSetMatrix(k, n, }\KeywordTok{sizeof}\NormalTok{(}\DataTypeTok{float}\NormalTok{), B, ldb, dB, dldb));}
    \NormalTok{CUBLAS_ERROR_CHECK(cublasSetMatrix(m, n, }\KeywordTok{sizeof}\NormalTok{(}\DataTypeTok{float}\NormalTok{), C, ldc, dC, dldc));}

    \CommentTok{// Perform the GPU SGEMM}
    \NormalTok{CUBLAS_ERROR_CHECK(cublasSgemm(handle,}
                                   \NormalTok{CUBLAS_OP_N, CUBLAS_OP_N,}
                                   \NormalTok{m, n, k,}
                                   \NormalTok{&a, dA, dlda, dB, dldb,}
                                   \NormalTok{&b, dC, dldc));}

    \CommentTok{// Copy results back into CPU memory}
    \NormalTok{CUBLAS_ERROR_CHECK(cublasGetMatrix(m, n, }\KeywordTok{sizeof}\NormalTok{(}\DataTypeTok{float}\NormalTok{), dC, dldc, C, ldc));}
\end{Highlighting}
\end{Shaded}
...
\subsubsection{OpenACC Pragmas}\label{openacc-pragmas}
...
\subsubsection{Clauses specific to pragma acc
...
\subsubsection{OpenACC SAXPY}\label{openacc-saxpy}
...
\DataTypeTok{void} \NormalTok{saxpy(}\DataTypeTok{int} \NormalTok{n, }\DataTypeTok{float} \NormalTok{a, }\DataTypeTok{const} \DataTypeTok{float} \NormalTok{* x, }\DataTypeTok{int} \NormalTok{incx, }\DataTypeTok{float} \NormalTok{restrict * y, }\DataTypeTok{int} \NormalTok{incy) \{}
\OtherTok{#pragma acc kernels}
  \KeywordTok{for} \NormalTok{(}\DataTypeTok{int} \NormalTok{i = }\DecValTok{0}\NormalTok{; i < n; ++i)}
    \NormalTok{y[i * incy] = a * x[i * incx] + y[i * incy];}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
\DataTypeTok{void} \NormalTok{sgemm(}\DataTypeTok{int} \NormalTok{m, }\DataTypeTok{int} \NormalTok{n, }\DataTypeTok{int} \NormalTok{k,}
           \DataTypeTok{float} \NormalTok{a, }\DataTypeTok{const} \DataTypeTok{float} \NormalTok{* restrict A, }\DataTypeTok{int} \NormalTok{lda, }\DataTypeTok{const} \DataTypeTok{float} \NormalTok{* restrict B, }\DataTypeTok{int} \NormalTok{ldb,}
           \DataTypeTok{float} \NormalTok{b, }\DataTypeTok{float} \NormalTok{restrict * C, }\DataTypeTok{int} \NormalTok{ldc) \{}
\OtherTok{#pragma acc kernels}
  \KeywordTok{for} \NormalTok{(}\DataTypeTok{int} \NormalTok{j = }\DecValTok{0}\NormalTok{; j < n; j++) \{}
      \KeywordTok{for} \NormalTok{(}\DataTypeTok{int} \NormalTok{i = }\DecValTok{0}\NormalTok{; i < m; i++)}
          \NormalTok{C[j * ldc + i] *= b;}

      \KeywordTok{for} \NormalTok{(}\DataTypeTok{int} \NormalTok{l = }\DecValTok{0}\NormalTok{; l < k; l++) \{}
          \DataTypeTok{const} \DataTypeTok{float} \NormalTok{temp = a * B[j * ldb + k];}
          \KeywordTok{for} \NormalTok{(}\DataTypeTok{int} \NormalTok{i = }\DecValTok{0}\NormalTok{; i < m; i++)}
              \NormalTok{C[j * ldc + i] += temp * A[l * lda + i];}
      \NormalTok{\}}
  \NormalTok{\}}
\NormalTok{\}}
\end{Highlighting}
...
\itemsep1pt\parskip0pt\parsep0pt
\item
  Further information (documentation/tutorials) is available on the
  (OpenACC website){[}http://www.openacc-standard.org/{]}
\end{itemize}

\subsection{CUDA Thrust}\label{cuda-thrust}

\subsubsection{CUDA Thrust}\label{cuda-thrust-1}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{// Creates an empty host vector to store double values}
\NormalTok{thrust::host_vector<}\DataTypeTok{double}\NormalTok{> F;}

\CommentTok{// Creates a host vector with space for 4 integers}
\NormalTok{thrust::host_vector<}\DataTypeTok{int}\NormalTok{> H(}\DecValTok{4}\NormalTok{);}

\CommentTok{// Creates a host vector containing 8 floats each initialised to 2.5}
\NormalTok{thrust::host_vector<}\DataTypeTok{float}\NormalTok{> J(}\DecValTok{8}\NormalTok{, }\FloatTok{2.5}\NormalTok{);}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{thrust::host_vector<}\DataTypeTok{int}\NormalTok{> H1;}
\NormalTok{thrust::host_vector<}\DataTypeTok{int}\NormalTok{> H2(H1);}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{thrust::host_vector<}\DataTypeTok{float}\NormalTok{> H(}\DecValTok{5}\NormalTok{);}
\NormalTok{thrust::host_vector<}\DataTypeTok{double}\NormalTok{> D(H);}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{thrust::host_vector<}\DataTypeTok{int}\NormalTok{> H;}
\NormalTok{thrust::device_vector<}\DataTypeTok{int}\NormalTok{> D(H);}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{std::vector<}\DataTypeTok{int}\NormalTok{> stl_vector;}
\NormalTok{thrust::device_vector<}\DataTypeTok{int}\NormalTok{> D(stl_vector);}
\end{Highlighting}
\end{Shaded}
...
\subsubsection{Vector types: Accessors}\label{vector-types-accessors}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{thrust::device_vector<}\DataTypeTok{int}\NormalTok{> D(}\DecValTok{4}\NormalTok{,}\DecValTok{0}\NormalTok{);}
\NormalTok{D[}\DecValTok{0}\NormalTok{] = }\DecValTok{99}\NormalTok{; }
\NormalTok{D[}\DecValTok{1}\NormalTok{] = }\DecValTok{88}\NormalTok{;}
\end{Highlighting}
\end{Shaded}
...
\itemsep1pt\parskip0pt\parsep0pt
\item
  The transform algorithm is declared in \texttt{thrust/transform.h}
\end{itemize}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{thrust::device_vector<}\DataTypeTok{int}\NormalTok{> X(}\DecValTok{10}\NormalTok{); }
\NormalTok{thrust::device_vector<}\DataTypeTok{int}\NormalTok{> Y(}\DecValTok{10}\NormalTok{); }

\CommentTok{// initialize X to 0,1,2,3, .... }
\NormalTok{thrust::sequence(X.begin(), X.end()); }

\CommentTok{//compute Y = -X }
\CommentTok{// thrust::transform(first, last, result, unary_op);}
\NormalTok{thrust::transform(X.begin(), X.end(), Y.begin(), thrust::negate<}\DataTypeTok{int}\NormalTok{>());}
\end{Highlighting}
\end{Shaded}
...
\itemsep1pt\parskip0pt\parsep0pt
\item
  \texttt{transform} applies a unary\_op functor to the elements between
  \texttt{first} and \texttt{last} and stores them in \texttt{result}
\end{itemize}

\subsubsection{Algorithms: reduce}\label{algorithms-reduce}

\begin{itemize}
\itemsep1pt\parskip0pt\parsep0pt
\item
  The reduction algorithm is declared in \texttt{thrust/reduce.h}
\end{itemize}

\begin{Shaded}
\begin{Highlighting}[]

\NormalTok{thrust::device_vector<}\DataTypeTok{int}\NormalTok{> vec(}\DecValTok{5}\NormalTok{,}\DecValTok{1}\NormalTok{);}
\CommentTok{//thrust::reduce(first, last, init, binary_op);}
\DataTypeTok{int} \NormalTok{sum = thrust::reduce(D.begin(), D.end(), (}\DataTypeTok{int}\NormalTok{) }\DecValTok{0}\NormalTok{, thrust::plus<}\DataTypeTok{int}\NormalTok{>());}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{thrust::device_vector<}\DataTypeTok{int}\NormalTok{> d(}\DecValTok{5}\NormalTok{)}
\NormalTok{thrust::negate<}\DataTypeTok{float}\NormalTok{> unary_op; }
\NormalTok{thrust::plus<}\DataTypeTok{float}\NormalTok{> binary_op; }
\DataTypeTok{float} \NormalTok{init = }\DecValTok{0}\NormalTok{;}

\NormalTok{thrust::transform_reduce(d.begin(), d.end(), unary_op, init, binary_op)}
\end{Highlighting}
\end{Shaded}
...
\KeywordTok{struct} \NormalTok{saxpy_functor : }\KeywordTok{public} \NormalTok{thrust::binary_function<}\DataTypeTok{float}\NormalTok{, }\DataTypeTok{float}\NormalTok{, }\DataTypeTok{float}\NormalTok{> \{}
    
    \DataTypeTok{const} \DataTypeTok{float} \NormalTok{a;}
    
    \NormalTok{saxpy_functor(}\DataTypeTok{float} \NormalTok{_a) : a(_a) \{\}}
    
    \NormalTok{__host__ __device__ }\DataTypeTok{float} \KeywordTok{operator}\NormalTok{()(}\DataTypeTok{const} \DataTypeTok{float} \NormalTok{&x, }\DataTypeTok{const} \DataTypeTok{float} \NormalTok{&y) }\DataTypeTok{const} \NormalTok{\{}
        \KeywordTok{return} \NormalTok{a * x + y;}
    \NormalTok{\}}
\NormalTok{\};}

\DataTypeTok{void} \NormalTok{saxpy(}\DataTypeTok{const} \DataTypeTok{float} \NormalTok{& a, }\DataTypeTok{const} \NormalTok{thrust::device_vector<}\DataTypeTok{float}\NormalTok{> & x, thrust::device_vector<}\DataTypeTok{float}\NormalTok{> & y) \{}
    \NormalTok{thrust::transform(x.begin(), x.end(), y.begin(), y.begin(), saxpy_functor(a));}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
\subsection{CUDA-C}\label{cuda-c}
...
\subsubsection{CUDA Function attributes}\label{cuda-function-attributes}
...
\begin{Highlighting}[]
        ...
        \NormalTok{saxpy<<<n/}\DecValTok{64} \NormalTok{+ }\DecValTok{1}\NormalTok{, }\DecValTok{64}\NormalTok{>>>(n, a, dx, incx, dy, incy);}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{__global__ }\DataTypeTok{void} \NormalTok{saxpy(}\DataTypeTok{int} \NormalTok{n, }\DataTypeTok{float} \NormalTok{a, }\DataTypeTok{const} \DataTypeTok{float} \NormalTok{* __restrict__ x, }\DataTypeTok{int} \NormalTok{incx,}
                      \DataTypeTok{float} \NormalTok{* __restrict__ y, }\DataTypeTok{int} \NormalTok{incy) \{}
    \DataTypeTok{const} \DataTypeTok{int} \NormalTok{i = blockIdx.x * blockDim.x + threadIdx.x;}
...
\subsubsection{Monte Carlo Pi}\label{monte-carlo-pi-1}
...
\NormalTok{$ }\KeywordTok{aws} \NormalTok{ec2 describe-images --owners amazon \textbackslash{}}
  \KeywordTok{--filters} \StringTok{"Name=name,Values=amzn-ami-hvm-2014.09.2.x86_64-ebs"}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# <AMI-ID>: ami-9d23aeea}
...
\begin{figure}[htbp]
\centering
\includegraphics{10Cloud/figures/notebook.png}
...
\begin{figure}[htbp]
\centering
\includegraphics{10Cloud/figures/dorian_wordcount.png}
\caption{Common words in Dorian Gray}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics{10Cloud/figures/dorian_dispersion.png}
...
\NormalTok{$ }\KeywordTok{cat} \NormalTok{dorian.txt }\KeywordTok{|} \KeywordTok{./mapper.py} \KeywordTok{|} \KeywordTok{sort} \KeywordTok{|} \KeywordTok{./reducer.py} \KeywordTok{|} \KeywordTok{more}
...
\NormalTok{$ }\KeywordTok{aws} \NormalTok{emr create-cluster --ami-version 3.1.0 \textbackslash{}}
    \KeywordTok{--ec2-attributes} \NormalTok{KeyName=}\KeywordTok{<}\NormalTok{key_name}\KeywordTok{>} \NormalTok{\textbackslash{}}
    \KeywordTok{--instance-groups} \NormalTok{InstanceGroupType=MASTER,InstanceCount=1,InstanceType=m3.xlarge \textbackslash{}}
    \OtherTok{InstanceGroupType=}\NormalTok{CORE,InstanceCount=}\KeywordTok{2}\NormalTok{,InstanceType=m3.xlarge}
...
\KeywordTok{export} \OtherTok{PATH=$PATH}\NormalTok{:/c/Program}\DataTypeTok{\textbackslash{} }\NormalTok{Files}\DataTypeTok{\textbackslash{} \textbackslash{}(}\NormalTok{x86}\DataTypeTok{\textbackslash{})}\NormalTok{/Git/bin}
